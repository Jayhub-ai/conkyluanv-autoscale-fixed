...
double max_upload_speed = 1e-47; /* Maximum value for upload speed graphs */
double max_download_speed = 1e-47; /* Maximum value for download speed graphs */
int current_graph_type = GRAPH_NONE; /* Track the current graph type being drawn */
...
static void graph_append(struct special_node *graph, double f, char showaslog) {
  ...
  if (showaslog != 0) {
#ifdef BUILD_MATH
    // Ensure the value is positive before taking the log
    if (f <= 0) {
      f = 1e-47; // Very small positive value to avoid log(0) which is -inf
    }
    f = log10(f);
    
    // Clamp negative log values to prevent drawing outside graph borders
    if (f < 0) {
      f = 0;
    }
#endif
  ...
  if (graph->scaled != 0) {
    ...
    if (graph->speedgraph) {
      if (current_graph_type == GRAPH_UPLOAD) {
        if (max_upload_speed < graph->scale) {
          max_upload_speed = graph->scale;
        }
        graph->scale = max_upload_speed;
        ...
        if (max_download_speed < graph->scale) {
          max_download_speed = graph->scale;
        }
        graph->scale = max_download_speed;
        ...
        // Legacy code for other graph types (backwards compatibility)
