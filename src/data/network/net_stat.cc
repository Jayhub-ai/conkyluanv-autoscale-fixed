...
double downspeedgraphval(struct text_object *obj) {
  auto *ns = static_cast<struct net_stat *>(obj->data.opaque);
  // Set a global variable or flag to indicate this is a download graph
  extern int current_graph_type;
  current_graph_type = GRAPH_DOWNLOAD;
  return (ns != nullptr ? ns->recv_speed : 0);
}

double upspeedgraphval(struct text_object *obj) {
  auto *ns = static_cast<struct net_stat *>(obj->data.opaque);
  // Set a global variable or flag to indicate this is an upload graph
  extern int current_graph_type;
  current_graph_type = GRAPH_UPLOAD;
