...
static inline void draw_graph_bars(special_node *current, std::unique_ptr<Colour[]>& tmpcolour, 
  ...
  if(graphheight > 0 && current->minheight - graphheight > 0) {
    ...
    graphheight = current->minheight;
  }
  
  // Ensure graphheight is not negative to prevent drawing outside bounds
  if (graphheight < 0) {
    graphheight = 0;
  }
  ...
  int offsety2 = current->inverty ? by + graphheight
                          : round_to_positive_int(static_cast<double>(by) + h - graphheight);
